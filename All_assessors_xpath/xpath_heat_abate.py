...
#    }
        def __new__(self, driver):
           ...
           # self.file = openpyxl.load_workbook("C:/Users/Muhammad Abbas Khan/Source/Repos/Automation-Zinpro/Excell_files/Locators.xlsx")
            ...
            data_click = { 'yes' : ( _global.WA_yes, _global.Warea_yes,
           ...
           """ 
            ...
            while (i!=22 and j!=12):
                ...
                """ store the data from conditions of excell sheet either the excell dropdown (yes/no) and store in path_yes"""
                _get_click_data = data_click[sheet.cell(i,7).value][j]
                ...
                if ((_get_click_data == _global.FL_FA_yes) or
                   ...
                   (_get_click_data == _global.Soak_RA_yes)):
                    ...
                    temp_space = data_keys[_get_click_data]
                   # print("send arrays")
                   # print(temp_space)
                    for k,l in zip(temp_space, range(i+11,i+13)):
                        
                      #  print("values")
                      #  print(sheet.cell(l,7).value)
                      # send_keys from Excell Cell
                     # sending keys from excell cell to Temp,Space text site field 
                        
                        self.driver.find_element(By.XPATH, k).send_keys(sheet.cell(l,7).value)
                    if _global.Soak_RA_yes in path_yes:
                        self.driver.find_element(By.XPATH, _global.Soak_time).send_keys(sheet.cell(30,7).value) 
                ...
                if ((_get_click_data == _global.FL_FA_yes) or
                    ...
                    self.selecting_dropdown(data_select[_get_click_data], sheet.cell(i+21,7).value, self.driver)
        ...
        def selecting_dropdown(xpath, drop_sheet_value, driver):
